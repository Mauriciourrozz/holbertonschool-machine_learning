    ...
    def train(self, X, Y, iterations=5000, alpha=0.05, verbose=True,
        ...
        Args:
            X (np.ndarray): Input data of shape (nx, m), where
                nx is the number of features and m is the number
                of examples.
            Y (np.ndarray): Correct labels of shape (1, m), with
                binary values 0 or 1.
            iterations (int): Number of iterations to train over.
            alpha (float): Learning rate for gradient descent.
            verbose (bool): If True, print cost every `step`
                iterations (including iteration 0 and the last).
            graph (bool): If True, plot training cost against
                iteration number after training completes.
            step (int): Interval (in iterations) at which to
                print and record cost. Must be > 0 and ≤ iterations.
        """
        ...
        # Para usar en el calculo de error con los pesos actuales
        A1, A2 = self.forward_prop(X)

        # Necesito solo A2, foward_prop me devuelve una tupla
        costoInicial = self.cost(Y, A2)
        iteraciones.append(0)
        costos.append(costoInicial)

        if verbose:
            print(f"Cost after 0 iterations: {costoInicial}")

        for i in range(1, iterations + 1):
            a1, a2 = self.forward_prop(X)
            self.gradient_descent(X, Y, a1, a2, alpha)
            if i % step == 0 or i == iterations:
                costoIteracion = self.cost(Y, a2)
                if verbose:
                    print(f"Cost after {i} iterations: {costoIteracion}")
                    iteraciones.append(i)
                    costos.append(costoIteracion)

        if graph:
            plt.title("Training Cost")
            plt.xlabel("iteration")
            plt.ylabel("cost")
            plt.plot(iteraciones, costos, 'b-')
            plt.show()
