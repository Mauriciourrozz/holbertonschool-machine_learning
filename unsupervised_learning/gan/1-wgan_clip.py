...
class WGAN_clip(keras.Model):
    ...
    generator : keras.Model
        Neural network that generates fake samples from latent vectors.
    ...
    real_examples : tf.Tensor
        Dataset of real samples for training.
    batch_size : int
        Number of samples per training batch.
    disc_iter : int
        Number of discriminator iterations per generator update.
    learning_rate : float
    ...
    def __init__(
        ...
        """
        Initialize the WGAN with generator, discriminator,
        ...
        disc_iter : int, optional
            Number of discriminator updates per training step
            (default is 2).
        ...
        # loss = mean(real_pred) - mean(fake_pred)
    ...
    def train_step(self, useless_argument):
        ...
        The discriminator is trained several times, applying weight
        clipping to enforce the Lipschitz constraint. Then the
        generator is trained once.
