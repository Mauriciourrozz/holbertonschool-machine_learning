...
class GaussianProcess:
    ...
    def update(self, X_new, Y_new):
        ...
        Arguments:
            X_new (numpy.ndarray): New entry point, shape (1,)
            Y_new (numpy.ndarray): New value of the function, form (1,)
        """
        # Agregar el nuevo punto a X y Y
        self.X = np.vstack((self.X, X_new.reshape(-1, 1)))
        self.Y = np.vstack((self.Y, Y_new.reshape(-1, 1)))

        # Calcular la covarianza entre los puntos existentes y el nuevo punto
        k_new = self.kernel(self.X[:-1], X_new.reshape(-1, 1))

        # Calcular la covarianza del nuevo punto consigo mismo
        k_xx = self.kernel(X_new.reshape(-1, 1), X_new.reshape(-1, 1))

        # Expandir la matriz K para incluir el nuevo punto
        self.K = np.vstack((
            np.hstack((self.K, k_new)),
            np.hstack((k_new.T, k_xx))
        ))
